...
const [ids , setIds] = useState("")
  const handleUpdateData = async () => {
    try {
      const docRef = doc(db, 'weight', ids); // Replace with your actual document ID
  ...
  return (
    <div className='w-full flex flex-col px-3'>
      ...
      <option key={1}>
          ...
          <option key={2} value='dt'>
          Danwila / Thibiripokuna
          </option>

          <option key={3} value='kh'>
          Kahatagaswewa
          </option>

          <option key={4} value='month6'>
          Below 6 Month
          </option>

          <option key={5} value='wl'>
          Welpothuwewa"
          </option>
